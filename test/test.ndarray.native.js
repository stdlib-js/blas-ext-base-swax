...
var swax = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 8', opts, function test( t ) {
	t.strictEqual( swax.length, 8, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	w = new Float32Array( x.length );
	out = swax( x.length, 3.0, x, 1, 0, w, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `w` unchanged', opts, function test( t ) {
	...
	swax( 0, 5.0, x, 1, 0, w, 1, 0 );
	t.deepEqual( w, expected, 'returns expected value' );

	swax( -4, 5.0, x, 1, 0, w, 1, 0 );
	t.deepEqual( w, expected, 'returns expected value' );
...
tape( 'the function supports negative strides', opts, function test( t ) {
	...
	swax( 3, 5.0, x, -2, x.length-1, w, -1, w.length-1 );
	t.deepEqual( w, expected, 'returns expected value' );
	t.end();
});
...
tape( 'if both strides are equal to `1`, the function efficiently multiplies each element of a strided array by a constant', opts, function test( t ) {
	...
	swax( x.length, alpha, x, 1, 0, w, 1, 0 );
	t.deepEqual( w, expected, 'returns expected value' );
